...
namespace bullet.fx.pack
{
    ...
    public sealed class Bullet : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            Transform visual = transform.Find("FireCylinder");
            if (visual != null)
            {
                visual.localRotation = Quaternion.Euler(90, 0, 0); // or try (0, 0, 90) if needed
            }
        ...
        private Vector3[] SetVertices()
        {
            ...
            Quaternion rotate90X = Quaternion.Euler(90f, 0f, 0f);
            for (int i = 0; i < originalVertices.Length; i++)
            {
                originalVertices[i] = rotate90X * originalVertices[i];
            }
