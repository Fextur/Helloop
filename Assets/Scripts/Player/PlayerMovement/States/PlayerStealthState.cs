using UnityEngine;
using Helloop.StateMachines;
using Helloop.Enemies;

namespace Helloop.Player.States
{
    public class PlayerStealthState : IState<PlayerMovement>
    {
        private float lastCameraCollisionCheck = 0f;
        private float cameraCollisionCheckInterval = 0.1f;

        public void OnEnter(PlayerMovement player)
        {
            currentPlayer = player;

            player.SwitchToTPSCamera();

            if (player.PlayerModel != null)
                player.PlayerModel.SetActive(true);

            if (player.Animator != null)
            {
                player.Animator.gameObject.SetActive(true);
                player.Animator.Play("CrouchIdle", 0, 0f);
            }

            if (player.TryGetComponent<WeaponManager>(out var weaponManager))
            {
                weaponManager.SetWeaponsActive(false);
                weaponManager.CanUseWeapons = false;
            }

            if (player.PlayerSystem != null)
            {
                player.PlayerSystem.SetStealth(true);
                player.PlayerSystem.OnPlayerDetected?.Subscribe(OnPlayerDetected);
            }
        }

        public void Update(PlayerMovement player)
        {
            HandleLook(player);
            HandleStealthMovement(player);
            HandleGravity(player);
            HandleCameraCollision(player);
        }

        public void OnExit(PlayerMovement player)
        {
            currentPlayer = null;

            player.SwitchToFPSCamera();

            if (player.PlayerModel != null)
                player.PlayerModel.SetActive(false);

            if (player.Animator != null)
                player.Animator.gameObject.SetActive(false);

            if (player.TryGetComponent<WeaponManager>(out var weaponManager))
            {
                weaponManager.SetWeaponsActive(true);
                weaponManager.CanUseWeapons = true;
            }

            if (player.PlayerSystem != null)
            {
                player.PlayerSystem.SetStealth(false);
                player.PlayerSystem.OnPlayerDetected?.Unsubscribe(OnPlayerDetected);
            }
        }

        private void HandleLook(PlayerMovement player)
        {
            if (player.GameStateSystem != null && player.GameStateSystem.isPaused)
                return;

            float mouseSensitivity = player.GetMouseSensitivity();
            float mouseX = Input.GetAxis("Mouse X") * mouseSensitivity * Time.deltaTime;
            float mouseY = Input.GetAxis("Mouse Y") * mouseSensitivity * Time.deltaTime;

            float currentFrameTime = Time.unscaledTime;
            float frameDelta = currentFrameTime - player.LastFrameTime;

            if (frameDelta > 0.1f)
            {
                player.LastFrameTime = currentFrameTime;
                return;
            }

            player.LastFrameTime = currentFrameTime;

            float newXRotation = player.XRotation - mouseY;
            newXRotation = Mathf.Clamp(newXRotation, -90f, 90f);

            float rotationDelta = Mathf.Abs(newXRotation - player.PreviousXRotation);
            if (rotationDelta > 45f)
            {
                return;
            }

            player.PreviousXRotation = player.XRotation;
            player.XRotation = newXRotation;

            player.TPSCamera.transform.localRotation = Quaternion.Euler(player.XRotation, 0f, 0f);
            player.transform.Rotate(Vector3.up * mouseX);
        }

        private void HandleStealthMovement(PlayerMovement player)
        {
            float h = Input.GetAxis("Horizontal");
            float v = Input.GetAxis("Vertical");

            Vector3 forward = player.FPSCamera.transform.forward;
            Vector3 right = player.FPSCamera.transform.right;
            forward.y = 0f;
            right.y = 0f;
            forward.Normalize();
            right.Normalize();

            Vector3 moveDirection = (forward * v + right * h).normalized;
            player.Controller.Move(moveDirection * player.stealthMoveSpeed * Time.deltaTime);

            if (player.Animator != null)
            {
                player.Animator.SetFloat("Speed", moveDirection.magnitude);
                player.Animator.SetFloat("Horizontal", h);
                player.Animator.SetFloat("Vertical", v);
            }
        }

        private void HandleGravity(PlayerMovement player)
        {
            player.CheckGrounded();

            if (player.IsGrounded && player.Velocity.y < 0)
            {
                player.Velocity = new Vector3(player.Velocity.x, -2f, player.Velocity.z);
            }

            player.Velocity = new Vector3(player.Velocity.x, player.Velocity.y + player.gravity * Time.deltaTime, player.Velocity.z);
            player.Controller.Move(player.Velocity * Time.deltaTime);
        }

        private void HandleCameraCollision(PlayerMovement player)
        {
            if (Time.time - lastCameraCollisionCheck >= cameraCollisionCheckInterval)
            {
                lastCameraCollisionCheck = Time.time;

                Vector3 flatForward = new Vector3(player.FPSCamera.transform.forward.x, 0f, player.FPSCamera.transform.forward.z).normalized;
                Vector3 desiredCameraPos = player.FPSCamera.transform.position + Vector3.up * 1f - flatForward * 3f;

                if (Physics.Linecast(player.FPSCamera.transform.position, desiredCameraPos, out RaycastHit hit))
                {
                    player.TPSCamera.transform.position = hit.point + player.TPSCamera.transform.forward * 0.3f;
                }
                else
                {
                    player.TPSCamera.transform.position = desiredCameraPos;
                }
            }
        }

        private void OnPlayerDetected()
        {
            if (currentPlayer != null)
            {
                currentPlayer.WantsToExitStealth = true;
            }
        }

        private PlayerMovement currentPlayer;
    }
}